...
for i in range(0,20):
    for j in range(0, 17):
        ...
        x3 = main_grid[i][j + 2]
        x4 = main_grid[i][j + 3]
        mult = int(x1) * int(x2) * int(x3) * int(x4)
        result.append(mult)
...
for i in range(0, 20):
    for j in range(0 ,17):
        ...
        y3 = main_grid[j + 2][i]        
        y4 = main_grid[j + 3][i]
        mult = int(y1) * int(y2) * int(y3) * int(y4)
        result.append(mult)
...
for i in range(0, 17):
    for j in range(0, 17):
        z1 = main_grid[i][j]
        z2 = main_grid[i + 1][j + 1]
        z3 = main_grid[i + 2][j + 2]
        z4 = main_grid[i + 3][j + 3]
        mult = int(z1) * int(z2) * int(z3) * int(z4)
        result.append(mult)
...
for i in range(0, 17):
    for j in range(3, 20):
        d1 = main_grid[i][j]
        d2 = main_grid[i + 1][j - 1]
        d3 = main_grid[i + 2][j - 2]
        d4 = main_grid[i + 3][j - 3]
        mult = int(d1) * int(d2) * int(d3) * int(d4)
        result.append(mult)
